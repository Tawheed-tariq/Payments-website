...
const Hero = () => {
  ...
  <section id="home" className={`flex md:flex-row flex-col ${styles.paddingY} `}>
    ...
    >
      <div
        className={`flex glow flex-row items-center py-[6px] px-4 bg-discount-gradient rounded-[10px] mb-2`}
      >
        <img src={discount} alt="discount" className="w-[32px] h-[32px]" />
        <p className={`${styles.paragraph} ml-2`}>
          <span className="text-white">20% </span>
          Discount For <span className="text-white">first Month </span>
        </p>
      </div>
      {/* Next genration payment methods     */}
      <div className="flex flex-row justify-between items-center w-full ">
        ...
        {/* Get started */}
        <div className={`ss:flex hidden md:mr-4 mr-0 `} data-aos="fade-up">
          <GetStarted />
        </div>
      </div>
      <h1
    ...
    <div className={`flex-1 flex ${styles.flexCenter} md:my-0  my-10 relative`}>
      <img src={robot} alt="billing" className="w-[100%] h-[100%] relative z-[5] " data-aos="zoom-in-left"/>
      {/* gradient start */}
